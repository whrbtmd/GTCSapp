...
jQuery().ready(function(){
 ...
 //TOP_RIGHT와 같은 위치로 지도상 어느부분에 위치하게되는지 결정된다 (googleMap ControlPosition or save-widget 참조)
   ...
   map.controls[google.maps.ControlPosition.RIGHT].push(plusTime);
 ...
 // Append a Save Control to the existing save-widget div.
   ...
   var time = new google.maps.SaveWidget(time);
   var plusTime = new google.maps.SaveWidget(plusTime);
     ...
     if (navigator.geolocation) {
         ...
         gps.getCurrentPosition(function (pos) {
           ...
           var college2 = [
            ...
            var college3 = [
              {lat: 35.894951, lng: 128.622057}, //left top
              {lat:35.895595, lng: 128.622057},
              {lat: 35.895595, lng: 128.623529},
              {lat: 35.894951, lng: 128.623529}
                  ];
  ...
  var playAlert = setInterval(function() {
        ...
        var p2 = pinMarkers[0].position;
            ...
            $(document).ready(function(){
                $('#result').empty();
                $('#result').append("<strong>" +result + " M</strong>");
                });
      ...
      function calcDistance(p1, p2) {
          return (google.maps.geometry.spherical.computeDistanceBetween(p1, p2)).toFixed(2);
             ...
             if(markerCount==1){
               if(m >= limitMinute){ //내가 제한시간을 초과했는가?
                 if(preState == "red"){ // 앞팀의 상태가 red라면
                   if(holeNum == preHole){ // 앞팀과 나의 홀 위치가 같다면
                     iconColor = "red";
                     markers[0].setIcon(blueIcon);
                     markers[0].setAnimation(null);
                   }else{
                     iconColor = "red";
                     markers[0].setIcon(redIcon);
                   ...
                   markers[0].setIcon(redIcon);
                   markers[0].setAnimation(google.maps.Animation.BOUNCE);
                   $('#map-canvas').css('border','solid 5px');
                   $('#map-canvas').css('border-color','#FF4848');
                   $('#plusTime').show();
                 }
